...
export async function POST(request: NextRequest) {
  try {
    ...
    // Send notifications
    try {
      const { fireAndForget } = await import('@/lib/concurrency')

      // Notify admins about new order
      fireAndForget(() => notifyNewOrder(order.id, shippingDetails.name, order.total))
      
      // Notify customer if logged in
      if (userId) {
        fireAndForget(() => notifyOrderConfirmed(userId, order.id))
      }

      // Email Notifications (Async & Non-Blocking)
      fireAndForget(async () => {
         const { sendOrderConfirmation, sendNewOrderSellerEmail } = await import('@/lib/email')
         ...
         await Promise.all(
           Object.entries(sellerEarnings).map(([email, amount]) => 
             sendNewOrderSellerEmail(email, order.orderNumber, amount)
           )
         )
      })
    } catch (notifyError) {
      console.log('Notification setup error (non-blocking):', notifyError)
    }
